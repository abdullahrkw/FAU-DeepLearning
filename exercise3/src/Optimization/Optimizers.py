...
class Sgd(Optimizer):
    ...
    def calculate_update(self,weight_tensor, gradient_tensor):
        if isinstance(self.regularizer, (L1_Regularizer, L2_Regularizer)):
            weight_tensor = weight_tensor - self.learning_rate*self.regularizer.calculate_gradient(weight_tensor) - self.learning_rate*gradient_tensor
        else:
...
class SgdWithMomentum(Optimizer):
    ...
    def calculate_update(self, weight_tensor, gradient_tensor):
        ...
        if isinstance(self.regularizer, (L1_Regularizer, L2_Regularizer)):
            updated_weight = weight_tensor + self.v - self.learning_rate*self.regularizer.calculate_gradient(weight_tensor)
        else:
...
class Adam(Optimizer):
    ...
    def calculate_update(self, weight_tensor, gradient_tensor):
        ...
        if isinstance(self.regularizer, (L1_Regularizer, L2_Regularizer)):
            updated_weight = weight_tensor - self.learning_rate*self.regularizer.calculate_gradient(weight_tensor) - self.learning_rate * (v_hat) / (np.sqrt(r_hat) + np.finfo(float).eps)
        else:
